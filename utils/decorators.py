...
def active_subscription_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not sub:
            allowed_endpoints = ['org.browse_plans', 'org.checkout', 'org.process_payment', 'org.logout']
            if request.endpoint not in allowed_endpoints:
                if getattr(current_user, 'role', '') == 'worker':
                    return redirect(url_for('main.subscription_expired'))
                
                flash("A subscription is required to access these features. Please choose a plan.", "warning")
                return redirect(url_for('org.browse_plans'))
            return f(*args, **kwargs)
            
        now = datetime.utcnow()
        # SHUTDOWN: Inactive status OR Expiry + 3 days grace
        is_expired_shutdown = sub.expires_at and now > (sub.expires_at + timedelta(days=3))
        ...
        if is_inactive or is_expired_shutdown:
            ...
            allowed_endpoints = ['org.browse_plans', 'org.checkout', 'org.process_payment', 'org.logout', 'main.subscription_expired', 'org.dashboard', 'org.profile']
            ...
            if request.endpoint not in allowed_endpoints:
                ...
                flash("Your subscription is completed/inactive. Please renew to continue services.", "warning")
                return redirect(url_for('org.dashboard'))
                
        # ALERT: If expired but within grace (3, 2, or 1 day remaining/past)
        if sub.expires_at and now > sub.expires_at and not is_inactive and not is_expired_shutdown:
            days_past = (now - sub.expires_at).days
            days_left = 3 - days_past
            flash(f"Your subscription has expired! You are in a grace period. {days_left} day(s) remaining before service interruption.", "warning")
