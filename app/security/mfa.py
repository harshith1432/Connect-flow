...
class MFAService:
    ...
    def is_mfa_enabled(user_id, user_type):
        config = MfaConfiguration.query.filter_by(
    ...
    def get_mfa_config(user_id, user_type):
        ...
        if not config:
            config = MfaConfiguration(
                user_id=user_id, user_type=user_type, mfa_type="none", is_enabled=False
            )
    ...
    def generate_and_send_otp(user_id, user_type, method="email"):
        ...
        if len(recent_otps) >= 3:
            sys.stderr.write(
                f"[MFA WARNING] Rate limit exceeded for OTP resend: user_type={user_type}, user_id={user_id}\n"
            )
            return (
                False,
                "Rate limit exceeded. Please wait a few minutes before requesting a new OTP.",
            )
    ...
    def _send_email_otp(email, code, name):
        ...
        print(
            f"[SMTP DISPATCH] Sending OTP to {email}: Hello {name}, your CRM security code is {code}.",
            flush=True,
        )
    ...
    def _send_sms_otp(phone, code, name):
        ...
        print(
            f"[SMS DISPATCH] Sending SMS to {phone} via Twilio: CRM Secure Code {code}.",
            flush=True,
        )
        ...
        if Config.TWILIO_ACCOUNT_SID and Config.TWILIO_AUTH_TOKEN:
            try:
                ...
                print(
                    f"[TWILIO SMS ERROR] Failed to send SMS via Twilio: {e}",
                    file=sys.stderr,
                    flush=True,
                )
    ...
    def verify_otp(user_id, user_type, code):
        ...
        if config and config.is_enabled and config.mfa_type == "totp":
            ...
            if totp.verify(
                code, valid_window=1
            ):  # 1 step window for network drift latency
                # Create Audit Log
        ...
        otp_records = (
            OtpVerification.query.filter_by(
                user_id=user_id, user_type=user_type, is_used=False
            )
        ...
        if check_password_hash(latest_otp.otp_code, code):
            ...
            MFAService._log_audit(
                ...
                user_type,
                "MFA Verification Success",
                f"OTP verified via {latest_otp.mfa_method}.",
            )
            return True, "MFA verified."
        ...
        MFAService._log_audit(
            ...
            user_type,
            "MFA Verification Failed",
            f"Invalid OTP entered (Attempt {latest_otp.attempts}/3).",
            severity="medium",
        )
        return (
            False,
            f"Invalid OTP code. You have {3 - latest_otp.attempts} attempts remaining.",
        )
