...
class SuspiciousActivityMonitor:
    ...
    def monitor_current_request(user_id, user_type):
        ...
        if active_sess.ip_address and active_sess.ip_address != current_ip:
            ...
            is_local_switch = (current_ip in ["127.0.0.1", "::1"]) and (
                active_sess.ip_address in ["127.0.0.1", "::1"]
            )

            if not is_local_switch:
                sys.stderr.write(
                    f"[SECURITY WARNING] IP switch detected for user {user_id} ({user_type})! Session IP: {active_sess.ip_address}, Request IP: {current_ip}\n"
                )
