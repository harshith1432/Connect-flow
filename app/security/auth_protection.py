...
class BruteForceProtection:
    ...
    def is_locked_out(identifier):
        ...
        lockout_time = datetime.utcnow() - timedelta(minutes=BruteForceProtection.LOCKOUT_MINUTES)
        ...
        if failed_count_ip >= BruteForceProtection.MAX_ATTEMPTS:
            ...
            suspicious = SuspiciousActivity.query.filter_by(
                ip_address=ip,
                activity_type="brute_force_lockout"
            ).filter(SuspiciousActivity.created_at >= lockout_time).first()
            ...
            return True, f"This IP has been temporarily locked out due to multiple failed login attempts. Please try again in {BruteForceProtection.LOCKOUT_MINUTES} minutes."
