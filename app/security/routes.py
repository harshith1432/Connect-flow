...
def verify_otp():
    ...
    masked_email = (
        email[:3] + "..." + email[email.find("@") - 2 :] if "@" in email else email
    )
    ...
    if request.method == "POST":
        ...
        if config.backup_codes and code in config.backup_codes:
            ...
            MFAService._log_audit(
                user_id,
                user_type,
                "Backup Code Verified",
                "User successfully authenticated with a backup recovery code.",
            )
        ...
        if success:
            ...
            MFAService._log_audit(
                ...
                user_type,
                "User Logged In",
                f"User logged in successfully via {mfa_method.upper()}.",
            )
...
def resend_otp():
    ...
    if mfa_method == "totp":
        return {
            "success": False,
            "message": "Resending is only supported for email or SMS OTPs.",
        }, 400

    success, msg = MFAService.generate_and_send_otp(
        user_id, user_type, method=mfa_method
    )
    ...
    if request.method == "POST":
        ...
        if action == "enable_email":
            ...
            MFAService._log_audit(
                ...
                user_type,
                "MFA Enabled",
                "Enabled Email-based Multi-Factor Authentication.",
            )
            ...
            if not phone:
                flash(
                    "SMS MFA requires a registered phone number. Please update your profile.",
                    "danger",
                )
                return redirect(url_for("security.mfa_setup"))
            config.mfa_type = "sms"
            ...
            MFAService._log_audit(
                ...
                user_type,
                "MFA Enabled",
                "Enabled SMS-based Multi-Factor Authentication.",
            )
            ...
            if totp.verify(totp_code, valid_window=1):
                ...
                MFAService._log_audit(
                    ...
                    user_type,
                    "MFA Enabled",
                    "Enabled Google Authenticator (TOTP) Multi-Factor Authentication.",
                )
                session["backup_codes_display"] = backup_codes
                flash(
                    "Google Authenticator setup verified and enabled successfully!",
                    "success",
                )
                return redirect(url_for("security.mfa_setup"))
            else:
                flash(
                    "Invalid Google Authenticator code. Please scan the QR code and try again.",
                    "danger",
                )
            ...
            if config.mfa_type == "totp" and config.is_enabled:
                ...
                flash(
                    "Backup recovery codes are only available for Authenticator (TOTP) MFA.",
                    "warning",
                )
...
def sessions():
    ...
    return render_template(
        "security/sessions.html", sessions=active_list, current_token=current_token
    )
...
def revoke_session(token):
    ...
    sess = ActiveSession.query.filter_by(
        session_token=token, user_id=user_id, user_type=user_type
    ).first()
    if not sess:
...
def audit_logs():
    ...
    # Query logs
    logs = (
        SecurityAuditLog.query.filter_by(user_id=user_id, user_type=user_type)
