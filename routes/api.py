...
def check_uniqueness():
    ...
    field = ModuleField.query.get_or_404(field_id)
    ...
    query = db.session.query(ModuleRecordValue).join(ModuleRecord).filter(
...
def twilio_message_status():
    ...
    if sid:
        ...
        if log:
            print(f"[TWILIO WEBHOOK] Found log for SID {sid}. Updating status to {status}")
            log.status = status
            if error_code:
