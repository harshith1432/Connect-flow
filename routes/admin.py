...
def configure_twilio(org_id):
    ...
    print(
        f"DEBUG: Received hooman_number='{hooman_number}', hooman_api_key='{hooman_api_key}'"
    )
    ...
    if hooman_api_key or hooman_number:
        print(f"DEBUG: Processing hooman_config update for org {org_id}")
        ...
        if updated_hooman:
            ...
            print(f"DEBUG: Assigned new hooman_config: {org.hooman_config}")
            ...
            exists = CommunicationNumber.query.filter_by(
                organization_id=org_id,
                number=hooman_number if hooman_number else hooman_cfg.get("number"),
            ).first()

            target_num = hooman_number if hooman_number else hooman_cfg.get("number")
            if target_num and not exists:
                cn = CommunicationNumber(
                    organization_id=org_id,
                    number=target_num,
        ...
        print(f"DEBUG: No Hooman Labs changes detected in form submission.")
    ...
    print(f"DEBUG: Final state before commit for org {org_id}:")
    print(f"  hooman_config: {org.hooman_config}")
    print(f"  twilio_config: {org.twilio_config}")
    ...
    try:
        ...
        db.session.rollback()
        print(f"ERROR: Failed to commit changes for org {org_id}: {str(e)}")
