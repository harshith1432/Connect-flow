...
class Organization(db.Model):
    ...
    allow_default_access = db.Column(db.Boolean, default=True) # Deprecated but kept for safety during migration
    whatsapp_channel_type = db.Column(db.String(50), default='whatsapp', nullable=False) # 'whatsapp' or 'whatsapp_business'
...
class OrganizationUser(UserMixin, db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('email', 'organization_id', name='_user_email_org_uc'),)
...
class Subscription(db.Model):
    ...
    status = db.Column(db.String(50), default='active') # active, past_due, canceled, suspended
...
class CommunicationNumber(db.Model):
    ...
    channel_type = db.Column(db.String(20), nullable=True, default='voice')  # 'voice' or 'whatsapp'
...
class ModuleGroup(db.Model):
    ...
    records = relationship('ModuleRecord', backref='group', cascade='all, delete-orphan')
...
class ModuleField(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    module_id = db.Column(db.Integer, db.ForeignKey('modules.id', ondelete='CASCADE'))
    group_id = db.Column(db.Integer, db.ForeignKey('module_groups.id', ondelete='CASCADE'), nullable=True)
    name = db.Column(db.String(255), nullable=False)
    field_type = db.Column(db.String(50), default='string')
    is_unique = db.Column(db.Boolean, default=False)
    meta = db.Column(db.JSON)

    group = relationship('ModuleGroup', backref=backref('fields', cascade='all, delete-orphan'))
...
class ModuleRecord(db.Model):
    ...
    values = relationship('ModuleRecordValue', backref='record', cascade='all, delete-orphan')


class ModuleRecordValue(db.Model):
    __tablename__ = 'module_record_values'
    id = db.Column(db.Integer, primary_key=True)
    record_id = db.Column(db.Integer, db.ForeignKey('module_records.id', ondelete='CASCADE'))
    field_id = db.Column(db.Integer, db.ForeignKey('module_fields.id', ondelete='CASCADE'))
...
class ContactGroupMap(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    group_id = db.Column(db.Integer, db.ForeignKey('contact_groups.id', ondelete='CASCADE'))
...
class Script(db.Model):
    ...
    type = db.Column(db.String(50), nullable=False)  # whatsapp_text, whatsapp_voice, call
    content = db.Column(db.Text, nullable=False)
    voice_content = db.Column(db.Text)  # Optional, for secondary script (e.g. voice part of whatsapp)
    meta = db.Column(db.JSON)
...
class Campaign(db.Model):
    ...
    sender_number_id = db.Column(db.Integer, db.ForeignKey('communication_numbers.id', ondelete='SET NULL'))
    filters = db.Column(db.JSON)
    status = db.Column(db.String(50), default='draft')
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    targets = relationship('CampaignTarget', backref='campaign', cascade='all, delete-orphan')
...
class DeliveryLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    campaign_id = db.Column(db.Integer, db.ForeignKey('campaigns.id', ondelete='SET NULL'))
...
class ChangeRequest(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('organization_users.id'), nullable=True)
    field_name = db.Column(db.String(100), nullable=False) # e.g., 'org_name', 'admin_email'
...
class PlatformNotification(db.Model):
    ...
    type = db.Column(db.String(50), nullable=False) # 'info_change', 'number_request', 'alert'
    ...
    organization_rel = relationship('Organization', backref=backref('notifications', cascade='all, delete-orphan'))
