...
if Config.TWILIO_ACCOUNT_SID and Config.TWILIO_AUTH_TOKEN:
    ...
    logger.warning('Default Twilio credentials not found; features disabled unless custom config used')
...
def _get_twilio_context(organization_id, channel='whatsapp', sender_number_id=None):
    ...
    if sender_number_id:
        ...
        if sender_record and sender_record.organization_id == organization_id and sender_record.active:
            if sender_record.is_platform_owned:
                print(f"DEBUG: Using Platform Default for number {sender_record.number}")
                client_instance = default_client
                sender_number = sender_record.number
                return client_instance, sender_number
            else:
                print(f"DEBUG: Using Custom DB Config for number {sender_record.number}")
                if custom_conf:
                    config_key = 'voice' if sender_record.channel_type in ['voice', 'indian_voice'] else 'whatsapp'
    ...
    if not client_instance:
        if org:
            ...
            if not allowed:
                 logger.info(f"Org {organization_id} has disabled platform default {channel} access. Blocking fallback.")
                 return None, None
...
def send_whatsapp_text(organization_id, contact_id, body=None, campaign_id=None, content_sid=None, content_variables=None, sender_number_id=None):
    """
    Send a WhatsApp message using Twilio. Supports raw body or Content API templates.
    """
    # 1. Log Initial Status
    log = DeliveryLog(campaign_id=campaign_id, contact_id=contact_id, channel='whatsapp', status='queued')
    db.session.add(log)
    db.session.commit()

    # 2. Get Client & Sender
    client, sender_number = _get_twilio_context(organization_id, channel='whatsapp', sender_number_id=sender_number_id)
        ...
        with app.app_context():
            ...
            try:
                if not thread_contact:
                    raise ValueError(f"Contact {contact_id} not found in background thread")
                ...
                from_ = sender_number if sender_number and sender_number.startswith('whatsapp:') else f'whatsapp:{sender_number}'
                
                # Status Callback URL
                callback_url = f"{Config.BASE_URL.rstrip('/')}/api/twilio/message-status"
                ...
                print(f"[DEBUG DISPATCH] Message Created! SID: {msg.sid}, Status: {msg.status}")
...
def send_whatsapp_voice(organization_id, contact_id, audio_url=None, tts_text=None, language='English', campaign_id=None, local_path=None, sender_number_id=None):
    """
    Send a WhatsApp voice note (audio) via Twilio by attaching `media_url`.
    """
    log = DeliveryLog(campaign_id=campaign_id, contact_id=contact_id, channel='whatsapp_voice', status='queued')
    db.session.add(log)
    db.session.commit()
    ...
    if not client:
        ...
        db.session.commit()
        logger.warning('Twilio send_whatsapp_voice attempted but client not configured')
        return log
    ...
    def _cleanup(path):
        """Delete local file after a delay to allow Twilio to fetch it"""
        import time
        import os
        ...
        try:
            if os.path.exists(path):
                os.remove(path)
                logger.info("Auto-cleaned voice note: %s", path)
        except Exception as e:
            logger.error("Failed to cleanup voice note %s: %s", path, e)
        ...
        with app.app_context():
            ...
            try:
                ...
                if audio_url:
                    ...
                    raise RuntimeError('TTS-to-audio not implemented.')
                
                callback_url = f"{Config.BASE_URL.rstrip('/')}/api/twilio/message-status"
                print(f"[DEBUG VOICE] Using Callback URL: {callback_url}")
                
                logger.info("Twilio voice send: to=%s, from=%s, media=%s", to, from_, media)
                ...
                print(f"[DEBUG VOICE] Message Created! SID: {msg.sid}, Status: {msg.status}")
                if thread_log:
                    thread_log.sid = msg.sid
                    thread_log.recipient = to_num
                    thread_log.status = msg.status
                    thread_log.meta = {'twilio_sid': msg.sid}
                    db.session.commit()
                    print(f"[DEBUG VOICE] Log updated in DB for ID: {thread_log.id}")
                
                # Start cleanup thread if local path exists
                if local_path:
                    threading.Thread(target=_cleanup, args=(local_path,), daemon=True).start()

            except Exception as e:
                if thread_log:
                    thread_log.status = 'failed'
                    thread_log.error = str(e)
                logger.error('Failed sending WhatsApp voice to contact %s: %s', contact_id, e)
            finally:
                db.session.commit()
...
def make_call(organization_id, contact_id, tts_text, language='English', campaign_id=None, sender_number_id=None):
    """
    Initiate a voice call via Twilio.
    """
    import traceback
    logger.info("make_call invoked for Org %s, Contact %s", organization_id, contact_id)
    print(f"DEBUG: make_call initiated for Org {organization_id}, Contact {contact_id}, Sender {sender_number_id}")
    
    log = DeliveryLog(campaign_id=campaign_id, contact_id=contact_id, channel='call', status='queued')
    db.session.add(log)
    db.session.commit()

    client, sender_number = _get_twilio_context(organization_id, channel='call', sender_number_id=sender_number_id)
    ...
    def _call():
        with app.app_context():
            ...
            try:
                ...
                logger.info("Twilio initiating call: to=%s, from=%s, lang=%s", to_num, from_raw, twilio_lang)
                print(f"DEBUG: Twilio calling {to_num} from {from_raw} with Text: {tts_text[:50]}...")
                ...
                print(f"[DEBUG CALL] Call Created! SID: {call.sid}, Status: {call.status}")
                if thread_log:
