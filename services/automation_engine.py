...
from models.models import ModuleRecord, ModuleRecordValue, ModuleField, Contact, Organization
...
class AutomationEngine:
    ...
    def recalculate_record(self, record_id):
        ...
        """
        record = ModuleRecord.query.get(record_id)
        if not record:
            return
        ...
        # Get current values
        current_values = {v.field_id: v.value for v in record.values}
        ...
        for f in fields:
            if f.field_type == 'boolean':
                ...
                if logic:
                    ...
                    self._update_record_value(record.id, f.id, "TRUE" if result else "FALSE")
                    context[f.name] = result
                    
        db.session.commit()

    def _update_record_value(self, record_id, field_id, value):
        rv = ModuleRecordValue.query.filter_by(record_id=record_id, field_id=field_id).first()
        if not rv:
