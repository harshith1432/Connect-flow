...
from models.models import Contact, DeliveryLog, Organization
...
def make_hooman_call(
    ...
    #    Priority: dedicated hooman_config column → legacy twilio_config['hooman_labs']
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Resolve 'from' number
    # Priority: org config number → sender_number_id (CommunicationNumber)
    # ------------------------------------------------------------------
    ...
    #    e.g. language="English" → config key "HOOMAN_AGENT_ENGLISH"
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    def _dispatch():
        # Dispatch immediately (removed hardcoded 9AM-9PM local wait)
        ...
        with app.app_context():
            ...
            try:
                # Build our public status-callback URL so Hooman/Plivo
                # will POST status updates back to our app
                try:
                ...
                if response.status_code in (200, 201, 202):
                    ...
                    try:
                        ...
                        # Capture task/call ID returned by Hooman (check both taskId and task_id)
                        thread_log.sid = str(
                        ...
                        pass  # response not JSON — not fatal
                else:
                    logger.error(
