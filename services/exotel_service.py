...
def _get_exotel_context(organization_id, sender_number_id=None):
    ...
    custom_conf = org.twilio_config.get('exotel', {}) if org and org.twilio_config else {}
...
def make_exotel_call(organization_id, contact_id, tts_text, language='English', campaign_id=None, sender_number_id=None):
    """
    Initiate a voice call via Exotel REST API.
    """
    logger.info("make_exotel_call invoked for Org %s, Contact %s, Sender %s", organization_id, contact_id, sender_number_id)
    
    log = DeliveryLog(campaign_id=campaign_id, contact_id=contact_id, channel='call', status='queued')
    db.session.add(log)
    db.session.commit()

    api_key, api_token, sender_number, subdomain = _get_exotel_context(organization_id, sender_number_id=sender_number_id)
    ...
    try:
        response = requests.post(url, auth=(api_key, api_token), data=payload, timeout=10)
        ...
        if response.status_code == 200:
            ...
            log.status = 'failed'
            log.error = res_data.get('RestException', {}).get('Message', 'Exotel API Error')
